...
검증 6개 레이어:
  ...
  Layer 5 — 파이프라인 일관성 : 산출물 시간/파일 존재, PM 조건 전항목
  Layer 6 — UX 검증          : 모바일 렌더링, 데이터 경고, HOLD 명확성

심각도:
  CRITICAL — 이 검증이 실패하면 output/ 저장을 차단 (exit 1)
  WARNING  — 플래그 표시하고 계속 진행 (사람이 확인 필요)
  INFO     — 참고 정보

Done Criteria (자체검증 의무):
  VA-1: CRITICAL 실패 0개일 때만 exit 0 (파이프라인 허가)
  VA-2: 30개 체크 전항목 실행 완료
  VA-3: L6 UX 레이어 포함 (모바일/경고/HOLD 검증)
