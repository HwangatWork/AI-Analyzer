# -*- coding: utf-8 -*-
"""
Phase 13-B-2 회귀 테스트: tf_schema_check.py (SubagentStop hook).

Tests:
T-TF-HSC-1: --selftest 모드 exit 0 + "PASS" stdout
T-TF-HSC-2: .active 플래그 없으면 즉시 exit 0 (no-op)
T-TF-HSC-3: _extract_json_block 이 fenced ```json 블록 파싱
T-TF-HSC-4: _extract_json_block 이 plain text 에서 None 반환
T-TF-HSC-5: _validate 가 invalid enum 거부
...
def _import_hook_module():
    """Load tf_schema_check.py as a Python module (no main() execution)."""
...
def test_T_TF_HSC_1_selftest_passes():
...
def test_T_TF_HSC_2_skip_when_inactive():
    """Hook exits 0 immediately when .active flag is absent (no TF in flight)."""
...
def test_T_TF_HSC_3_extract_fenced_json():
...
def test_T_TF_HSC_4_extract_returns_none_for_plain_text():
...
def test_T_TF_HSC_5_validate_rejects_invalid_enum():
