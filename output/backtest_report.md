...
| Test | 결과 (요약) | 정직 판정 |
|------|-------------|-----------|
| **P-4** | Strategy 0% cumret vs SP500 BH +0.14% / KOSPI BH -10.65%. 시그널 12개 스냅샷 전체에서 BUY 미발생 → 전략은 구조적으로 100% 현금 유지 | **미판정 (구조적 편향)** — 시그널이 매수 신호를 만들지 않는 리스크-오프 국면에서만 실행됨 |
| **P-1** | Top5 hit rate 36.7% (11/30) < Bottom5 60.0% (18/30). Δ=-23.3pp on 5d SP500 forward direction, N_snap=6 | **경고 신호** — 표본 30 obs 로 CI 겹침(추론 불가)이지만 방향 자체가 weight ranking 예측력에 반대 |
| **P-2** | Mean precision@5 = 0.533 < chance baseline 0.625 (universe=8, N_snap=6) | **미달** — 좁은 universe 로 baseline 이 부풀려졌지만 예측이 chance 이하 |
| **P-3** | SP500: 필터 후 N_evaluable=0 (100% top5 가 warn_reason). KOSPI: N=2, mean excess -0.019%, win rate 50% | **산출물 사용 불가** — beneficiary_top5 가 pipeline 자체 warn 으로 포화 |
...
2. 초기 상태 flat 규칙 하에 전략은 12일 내내 100% 현금 → cumret 0.000%.
3. **KOSPI 하락(-10.65%)은 회피**했지만 이는 예측력이 아니라 "매수 시그널이 없으면 진입 안 함" 이라는 구조적 결과.
4. **SP500 상승(+0.14%)은 놓쳤음**. 절대 손실은 작지만 리스크-오프 편향의 대가.
5. Confidence gate variants (60/70) 는 모두 동일 결과 — BUY 시그널이 없으므로 gate 는 무의미.
6. Sharpe 는 std=0 → 정의 불가. N=9~11 거래일로 통계적 Sharpe 는 SE≈1.6 (peer review 명시).
...
**정직 발견**:
1. 프롬프트 원래 성공 기준 (상위 그룹 > 하위 그룹, p<0.05, N≥30) — **미달**. Δ 방향이 예상과 반대이며 Wilson CI 가 겹쳐 통계적 결론 불가.
...
4. 6 스냅샷 × 5 인디케이터 = 30 obs 는 프롬프트의 최소 30 요건은 만족하나, 스냅샷별 독립성 문제(같은 지표가 여러 스냅샷에 반복) 로 실질적 유효 표본은 훨씬 작음.
...
**정직 발견**:
1. Mean precision@5 (0.533) < chance baseline (0.625). 예측이 무작위 이하.
...
3. Universe 를 S&P500 전체 (500 종목) 로 확장하면 chance baseline 은 5/500 = 1% 로 급락 → 그 경우 precision 0.4~0.8 은 매우 유의미한 시그널. 그러나 500 종목 시가총액 가중 기여를 매 스냅샷마다 forward 계산하는 것은 인프라 추가에 해당해 이번 범위 밖.
