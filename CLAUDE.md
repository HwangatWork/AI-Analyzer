...
- Regression baseline: 92+ PASS, ≤1 SKIP, 0 FAIL minimum. (현재 관측: 93 PASS, 0 SKIP, ~59초 — T23 환경 영향) Never merge below this. Push 전 반드시 로컬에서 `pytest agents/tests/ -v --tb=short -q -x` 실행 확인. (Phase 13-B-5에서 `regression_baseline.json` 신설 후 single source of truth 전환 예정)
