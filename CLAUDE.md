...
## agents/ 폴더 구조 (Phase 0 정리 후 — 2026-06-11)

**에이전트 명세는 `.claude/agents/`가 유일한 진실 소스다.** `agents/*.md`는 삭제됨.

### 파일 분류 기준
| 그룹 | 위치 | 기준 |
|------|------|------|
| active | `agents/` 루트 | PIPELINE_STAGES 등록 + infra(pm/stat_utils/utf8_setup) + UX 서브에이전트(run_ux_*.py) + 조건부(run_telegram_agent.py, run_notion_agent.py) |
| standby | `agents/standby/` | credentials 미설정 또는 파이프라인 미연결 — run_sheets_agent.py, run_ctd_integration_agent.py |
| dead/v1 | `agents/archive/` | 구버전(v1) — run_data/analysis/evaluator/stock_agent.py(v1) |

SD-10/11/12 및 SA 전체는 PIPELINE_STAGES 기준 active 그룹만 스캔한다 (standby/archive 제외).
...
**게이트 완료 기준 불변 규칙 (2026-06-11 추가):**
- **게이트는 모든 조건에 수치/로그 Evidence가 Telegram 전송될 때까지 통과로 처리하지 않는다.** 조건별 Evidence 없이 완료 보고 = 자동 게이트 실패.
- **보고서에서 게이트 조건 하나라도 누락하면 게이트 실패다.** 누락된 조건은 "검증 불가"로 처리하고 게이트를 재실행해야 한다.
...
- QG-1(2026-06-10): 선택 기능(Google Sheets 등)을 필수 기능과 동일한 PASS/FAIL 이진 구조로 체크 → 미설정=FAIL 오탐. **새 외부 연동 체크 추가 시 반드시 optional 3-state 패턴 적용**: 미설정→SKIP(soft-pass) / 설정+오류→FAIL / 설정+정상→PASS. `_KNOWN_FAIL_CHECKS`는 임시방편, 체크 자체를 optional로 설계해야 함.
- SA-7c(2026-06-10): KOSPI 기여 Top5에 삼성전자·SK하이닉스가 누락 — 두 종목 가격 수집은 성공했으나 이전 실행에서 기여 점수가 stock_results.json에 포함되지 않은 채로 downstream에 전파됨. **파이프라인 산출물 품질은 코드 구조와 별도로 pm_system_audit SA-7로 주기적 검증 필수**. 시총 상위 종목이 기여 Top5 부재 + 시총 비율 30x 초과 = CRITICAL. fix 시 `warn_reason` 필드 추가 의무 (극단 수익률 500%+). 새 종목 추가 시 기존 ticker 덮어쓰기 로직 필요 (dedup만으로는 기존 항목 갱신 불가).
- Phase-0(2026-06-11): agents/standby/로 이동된 파일을 run_ui_agent.py가 하드 임포트 → ModuleNotFoundError. **에이전트를 standby/로 이동할 때, 해당 에이전트를 임포트하는 모든 상위 파일의 import 구문을 반드시 conditional import(try/except)로 전환해야 한다.**
- stop_hook Check2 SKIP 패턴(2026-06-11): 실전 transcript에서 last_user가 tool_result 블록만 있어 빈 문자열이 되거나, 레벨 지정이 영어("Level 10")로 돼있어 한국어 패턴(`레벨\s*([789]|10)`)이 미매칭됨 → Check2 SKIP 오반환. **recent_level_ctx 스캔 한도(10개)가 긴 세션에서 불충분 + 영어 Level 패턴 미지원**이 복합 원인.
- FIX-D stop_hook JSONL 형식 미지원(2026-06-11): 실제 Claude Code hook stdin transcript는 각 항목이 `{"type":"user","message":{"role":"user","content":"..."}}` JSONL 형식이지만, 코드가 `msg.get("role","")` 최상위에서 role 탐색 → 항상 "" 반환 → last_user=last_asst="" → Check1/2=SKIP, task_hint="(작업 내용 없음)". **selftest_transcript.json이 clean 형식({role,content})만 사용해 실전 버그 재현 불가**. 수정: `_normalize_msg()` 헬퍼로 JSONL/clean 양형식 지원. selftest_transcript.json도 JSONL 형식으로 업데이트해 실전 동작을 검증해야 함.
- SA-1 false positive 패턴(2026-06-11): CONTEMPORANEOUS 배제 집합(`_CONTEMPORANEOUS = {"KOSDAQ",...}`) 정의 라인이 SA-1 스캔에 걸려 CRITICAL 오탐. **SA-1 체크에서 `_CONTEMPORANEOUS` 변수명이 포함된 라인은 배제 목적 정의이므로 건너뛰어야 함.** 수정: 라인-by-라인 스캔에서 `_CONTEMPORANEOUS` 포함 라인 skip.
